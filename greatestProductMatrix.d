/*
 * What is the greatest product of four adjacent numbers in any direction (up, down, left, right, or diagonally) in the 20x20 grid? (Euler 11)
...
void main() {
	...
	for(int i = 0; i < 17; i++)
		for(int j = 0; j < 17; j++) {
			temp = matrix[i][j] * matrix[i + 1][j + 1] * matrix[i + 2][j + 2] * matrix[i + 3][j + 3]; //Right
			if(temp > max)
				max = temp;
			temp = matrix[i][j + 3] * matrix[i + 1][j + 2] * matrix[i + 2][j + 1] * matrix[i + 3][j]; //Left
			if(temp > max)
				max = temp;
		}
	writeln("11: ", max);
